...
| Audit tool safety | 7/7 Python static-crawler/report safety tests и 6/6 Node browser-safety tests; реальные fixture-crawls проверяют canonical aliases/ports, form endpoint scope, social URLs и allowlisted URL-bearing поля JSON-LD, а также точный Docker image fallback; browser resolver отклоняет realpath/junction escapes. |
...
| Static crawler | PASS: 55 HTML без Yandex verification; 52 indexable, 2 noindex, 1 error page; 2 914 references, включая 268 значений из allowlisted URL-bearing полей JSON-LD и 27 social-image meta, 49 DOM fragments, 123 `<img>` и 124 JSON-LD blocks; 52 canonical точно совпадают с 52 sitemap URLs; robots.txt содержит 10 групп User-agent. |
...
| Противоречивые цены, сроки, скидка, «без откачки», псевдолокальные кейсы/отзывы и география офисов | Неподтверждённые утверждения удалены, цены ограничены подтверждённым составом, городские тексты нейтрализованы: `937b811`, `92f2f9a`, `a63bdc3`, `d8119a6`, `c860112`, `8fd6027`, `1bed860` | Content/SEO/GEO contracts входят в 113 Python tests; crawler: 52 canonical/sitemap и 295 social/allowlisted structured URL references; browser: 55 страниц | Андрей: закрыть реестр фактов; до этого удалённые claims не возвращать. |
...
| Audit tooling не проверял social-image и schema URLs, принимал слишком широкий repository image fallback и пропускал `www` alias | Crawler и browser runner приведены к Docker contract: `html/images` имеет приоритет, только `/images/portfolio/**` получает fallback; отсутствующий `/images/og-image.jpg` заменён опубликованными тематическими URL; JSON-LD обход ограничен allowlist URL-bearing ключей, поддерживает относительные IRI и полностью исключает словарный `@context` | 7/7 crawler/report contracts; 6/6 browser safety; static crawl 2 914 references; Chromium 220/220 | При изменении Docker `COPY` синхронно обновлять resolver и его контрактные тесты. |
...
- **SEO/GEO:** 52 индексируемые canonical соответствуют sitemap один-к-одному; локальные ссылки, фрагменты, изображения и значения из allowlisted URL-bearing полей JSON-LD разрешаются. Страницы сохраняют работу по всему Крыму и не изображают отдельные офисы/кейсы без доказательств.
